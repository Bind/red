...
export function assertHealthContract(
  ...
  if (record.status !== "ok" && record.status !== "degraded" && record.status !== "error") {
    throw new Error(`health body.status must be one of ok|degraded|error, got ${record.status}`);
  }
  ...
  if (expectedService && record.service !== expectedService) {
    throw new Error(`health body.service must be '${expectedService}', got '${record.service}'`);
  }
}
