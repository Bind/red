...
We now have a minimal Flue-backed entrypoint for the shared review engine.

Files:

- `.flue/agents/daemon-review.ts`
- `workflows/daemon-review/src/flue-run.ts`

Current intent:

- treat Flue as an alternate outer harness
- keep the shared review engine in `src/core.ts`
- keep GitHub publishing in `src/github.ts`
- reuse the same local artifact-writing path as `src/local.ts`

This is deliberately not a full migration of daemon execution into Flue yet.
The current spike is here to prove:

- the shared review engine can run under `flue run`
- local and CI-style invocation can share one harness shape
- we can add a future Flue execution backend without rewriting the GitHub
  adapter

Run it locally with:

```bash
just daemon-review-flue --base HEAD~1 --head HEAD
just daemon-review-flue --base HEAD~1 --head HEAD --daemon compose-contract
```

The Flue path currently uses `sandbox: "local"` and still relies on our
existing daemon review engine for routing, execution, artifact generation, and
blocking semantics.

Current limitation:

- Flue's Node bundling path does not currently accept our Bun-specific imports
  (`import { S3Client, write } from "bun"` inside the routing stack), so the
  spike agent shells into the existing Bun local entrypoint instead of importing
  `src/core.ts` directly.
- That is acceptable for now because it still validates the outer orchestration
  shape (`flue run` + local sandbox + shared runner behavior) without forcing a
  wider runtime portability rewrite.
