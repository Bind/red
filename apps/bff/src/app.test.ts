...
describe("BFF app", () => {
  ...
  test("RED_ADMIN_TOKEN forwards admin bearer instead of session-exchange", async () => {
    ...
    const app = createApp({
      ...
      fetchImpl: async (input, init) => {
        ...
        if (url.pathname === "/api/velocity") {
          ...
          return Response.json({ summarized: 0, pending_review: 0 });
        }
        return new Response("not found", { status: 404 });
      },
    });

    const res = await app.request("http://bff.test/rpc/velocity");
    expect(res.status).toBe(200);
    expect(sessionExchangeCalls).toBe(0);
    expect(upstreamAuth).toBe("Bearer shhh-admin");
  });

  test("RED_ADMIN_TOKEN bypasses requireSession on internal proxies", async () => {
    let sessionExchangeCalls = 0;
    const app = createApp({
      port: 3001,
      apiBaseUrl: "http://api.test",
    ...
    const res = await app.request("http://bff.test/rpc/rollups");
    expect(res.status).toBe(200);
    expect(sessionExchangeCalls).toBe(0);
