...
export class StubTriageWorkflowRunner implements TriageWorkflowRunner {
  ...
  start(
    ...
    const errorName = typeof primary.name === "string" ? primary.name : "UnknownError";
    const errorMessage = typeof primary.message === "string" ? primary.message : "";

    const plan: TriagePlan = {
      hypothesis: `${errorName} in ${rollup.entry_service}: ${errorMessage}`,
      suspected_files: [],
      reproduction_steps: [`Replay request ${rollup.request_id} against ${rollup.entry_service}`],
      proposed_change_summary: "Placeholder plan — stub runner returns immediately for local dev.",
      confidence: "low",
    };
