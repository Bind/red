...
import type { TriageWorkflowEvent, TriageWorkflowHandle, TriageWorkflowRunner } from "./runner";
import type { SmithersHttpClient } from "./smithers-client";
      ...
      try {
        for await (const event of this.client.events(runId, {
          ...
          if (event.type === "RunFailed" || event.type === "RunCancelled") {
            const message = typeof event.error === "string" ? event.error : `run ${event.type}`;
