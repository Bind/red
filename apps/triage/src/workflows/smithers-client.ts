...
export class SmithersHttpClient {
  ...
  async approve(
    ...
    await this.request("POST", `/v1/runs/${runId}/nodes/${nodeId}/approve`, body);
  }
  ...
  async deny(
    ...
    await this.request("POST", `/v1/runs/${runId}/nodes/${nodeId}/deny`, body);
  }
  ...
  async *events(
    ...
    const params = options.afterSeq !== undefined ? `?afterSeq=${options.afterSeq}` : "";
    const res = await this.fetchImpl(`${this.baseUrl}/v1/runs/${runId}/events${params}`, {
      method: "GET",
      headers: this.headers({ accept: "text/event-stream" }),
      signal: options.signal,
    });
    if (!res.ok || !res.body) {
      throw new Error(`smithers events stream failed: ${res.status} ${res.statusText}`);
    }
  ...
  private async request(method: string, path: string, body?: unknown): Promise<Response> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`smithers ${method} ${path} failed: ${res.status} ${res.statusText} ${text}`);
    }
    return res;
  }
  ...
  private readLatestRow(runId: string, tableName: string, nodeId: string): unknown {
    ...
    try {
      ...
      if (!row) {
        throw new Error(`no row in ${tableName} for run ${runId} node ${nodeId}`);
      }
