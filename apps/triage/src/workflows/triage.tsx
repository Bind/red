...
import { approvalDecisionSchema, createSmithers, Loop, Sequence } from "smithers-orchestrator";
import {
  ReviewFixSchema,
  ReviewSchema,
  TriagePlanSchema,
...
const { Workflow, Task, Approval, smithers, tables, outputs, useCtx } = createSmithers({
...
Issues to address:     ${
        lastReview && !lastReview.approved ? JSON.stringify(lastReview.issues) : "none"
      }
...
function InvestigateReviewFix() {
  ...
  return (
    <Task id="investigate-review-fix" output={outputs.reviewFix} agent={triageAgent} skipIf={skip}>
...
Previous iteration:
  ...
  review:   ${lastReview && !lastReview.approved ? JSON.stringify(lastReview.issues) : "none"}
...
function ProposeValidate() {
  return (
    <Task id="validate" output={outputs.validate} agent={triageAgent} timeoutMs={10 * 60 * 1000}>
...
function ProposeReviewFix() {
  ...
  const skip = !review || review.approved || review.issues.length === 0;
  return (
    <Task id="propose-review-fix" output={outputs.reviewFix} agent={triageAgent} skipIf={skip}>
