...
import { type ServerOptions, startServer } from "smithers-orchestrator";

const port = Number.parseInt(process.env.SMITHERS_SERVER_PORT ?? "7331", 10);
const authToken = process.env.SMITHERS_API_KEY;
const dbPath = resolve(process.env.SMITHERS_DB_PATH ?? "/smithers-data/smithers.db");
...
logger.info("smithers server listening on {url}", {
  url: `http://0.0.0.0:${port}`,
  db_path: dbPath,
  root_dir: rootDir,
  auth: authToken ? "enabled" : "disabled (no SMITHERS_API_KEY)",
