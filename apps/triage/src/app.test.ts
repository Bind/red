...
describe("triage app lifecycle", () => {
  test("receive → plan_ready → approve → proposing → proposal_ready", async () => {
    ...
    await waitFor(() => (store.get(id)?.status === "investigating" ? true : undefined));
    ...
    await waitFor(() => (store.get(id)?.status === "plan_ready" ? true : undefined));
    ...
    await waitFor(() => (store.get(id)?.status === "proposing" ? true : undefined));
