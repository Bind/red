...
export function createApp(deps: TriageAppDeps) {
  return new Hono()
    .use("*", createHttpLogger({ service: "triage", app: "red" }))
    .get("/health", (c) => {
      const health = buildHealth({ service: "triage" });
      return c.json(health, statusHttpCode(health.status));
    })
    .post("/v1/runs", async (c) => {
    ...
    .get("/v1/runs", (c) => c.json({ runs: deps.store.list() }))
    .get("/v1/runs/:id", (c) => {
      const run = deps.store.get(c.req.param("id"));
      if (!run) return c.json({ error: "not found" }, 404);
      return c.json(run);
    })
    .post("/v1/runs/:id/approve", async (c) => {
      try {
    ...
    .post("/v1/runs/:id/reject", async (c) => {
      try {
