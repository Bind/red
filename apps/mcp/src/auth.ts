...
export class OAuthIntrospector {
  private readonly cache = new Map<string, { result: IntrospectionResult; expiresAtMs: number }>();
  ...
  async introspect(token: string): Promise<IntrospectionResult> {
    ...
    const response = await this.fetchImpl(`${this.config.authBaseUrl}/oauth/introspect`, {
      method: "POST",
      headers: {
    ...
    if (result.active) {
      const expiresAtMs = result.exp ? Math.min(result.exp * 1000, nowMs + 60_000) : nowMs + 60_000;
...
export function oauthMiddleware(
  ...
  return async (c, next) => {
    ...
    if (config.requiredScope && !(result.scope ?? "").split(/\s+/).includes(config.requiredScope)) {
