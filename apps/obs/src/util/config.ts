...
function loadS3Config(env: NodeJS.ProcessEnv, prefix: "RAW" | "ROLLUP"): S3StorageConfig {
  return {
    endpoint: requiredString(env.WIDE_EVENTS_S3_ENDPOINT, "WIDE_EVENTS_S3_ENDPOINT"),
    region: requiredString(env.WIDE_EVENTS_S3_REGION, "WIDE_EVENTS_S3_REGION"),
    accessKeyId: requiredString(env.WIDE_EVENTS_S3_ACCESS_KEY_ID, "WIDE_EVENTS_S3_ACCESS_KEY_ID"),
    secretAccessKey: requiredString(
      env.WIDE_EVENTS_S3_SECRET_ACCESS_KEY,
      "WIDE_EVENTS_S3_SECRET_ACCESS_KEY",
    ),
    bucket: requiredString(env[`WIDE_EVENTS_${prefix}_BUCKET`], `WIDE_EVENTS_${prefix}_BUCKET`),
    prefix: requiredString(env[`WIDE_EVENTS_${prefix}_PREFIX`], `WIDE_EVENTS_${prefix}_PREFIX`),
  };
}

export function loadConfig(env: NodeJS.ProcessEnv = process.env): WideEventsConfig {
  ...
  if (storageBackendValue !== "file" && storageBackendValue !== "minio") {
    throw new Error("WIDE_EVENTS_STORAGE_BACKEND must be either 'file' or 'minio'");
  }
  const storageBackend = storageBackendValue as StorageBackend;
  const dataDir = resolve(requiredString(env.WIDE_EVENTS_DATA_DIR, "WIDE_EVENTS_DATA_DIR"));
  const rawEventsDir = resolve(
    requiredString(env.WIDE_EVENTS_RAW_EVENTS_DIR, "WIDE_EVENTS_RAW_EVENTS_DIR"),
  );
  const rollupDir = resolve(requiredString(env.WIDE_EVENTS_ROLLUP_DIR, "WIDE_EVENTS_ROLLUP_DIR"));
  ...
  return {
    ...
    replayWindowMs,
    storageBackend,
    rawS3: storageBackend === "minio" ? loadS3Config(env, "RAW") : undefined,
    rollupS3: storageBackend === "minio" ? loadS3Config(env, "ROLLUP") : undefined,
...
export function createStores(
  ...
  if (config.storageBackend === "minio") {
    if (!config.rawS3 || !config.rollupS3) {
      throw new Error("MinIO storage backend requires rawS3 and rollupS3 config");
    }
    return {
