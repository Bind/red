...
function loadConfig(env: NodeJS.ProcessEnv = process.env): SyncConfig {
  return {
    ...
    accessKeyId: required(env.WIDE_EVENTS_S3_ACCESS_KEY_ID, "WIDE_EVENTS_S3_ACCESS_KEY_ID"),
    ...
    outputDir: resolve(
      env.WIDE_EVENTS_AGENT_ROLLUP_CACHE_DIR ?? ".wide-events-agent/materialized/rollups",
    ),
  };
}
