...
import type { AcceptedCollectorBatch, RawEventStore } from "../service/collector-service";
...
export class FileRawEventStore implements RawEventStore {
  ...
  appendBatch(batch: AcceptedCollectorBatch): void {
    ...
    for (const event of batch.events) {
      const filePath = rawPath(this.rootDir, partitionDate(event.ts), event.service);
