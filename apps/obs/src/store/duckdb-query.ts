...
import type { RollupListOptions, RollupStore } from "../service/collector-service";
import type { S3StorageConfig } from "../util/s3";

export interface DuckDbQueryConfig {
  /** Where to read rollup NDJSON from. */
  source: { kind: "file"; rootDir: string } | { kind: "s3"; s3: S3StorageConfig };
}

export type AggregateKey = "entry_service" | "route" | "final_outcome" | "error_name";
...
export class DuckDbRollupQuery implements RollupStore {
  ...
  private async init(): Promise<void> {
    ...
    if (this.config.source.kind === "s3") {
      ...
      const stmts: string[] = [
        `SET s3_region='${escapeSqlLiteral(s3.region)}'`,
        `SET s3_access_key_id='${escapeSqlLiteral(s3.accessKeyId)}'`,
        `SET s3_secret_access_key='${escapeSqlLiteral(s3.secretAccessKey)}'`,
        `SET s3_endpoint='${escapeSqlLiteral(url.host)}'`,
  ...
  async listRollups(options: RollupListOptions = {}): Promise<WideRollupRecord[]> {
    ...
    const sql = `
      ...
      FROM read_ndjson_auto('${escapeSqlLiteral(this.rollupPath())}')
