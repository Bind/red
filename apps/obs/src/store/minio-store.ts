...
import type { WideCollectorEvent, WideRollupRecord } from "../service/collector-contract";
import type {
  AcceptedCollectorBatch,
  RawEventStore,
  RollupStore,
} from "../service/collector-service";
import { createS3Client, datePartitionsBetween, joinKey, type S3StorageConfig } from "../util/s3";
...
export class MinioRawEventStore implements RawEventStore {
  ...
  async appendBatch(batch: AcceptedCollectorBatch): Promise<void> {
    await Promise.all(
      batch.events.map((event) =>
        this.client.write(rawKey(this.prefix, event), `${JSON.stringify(event)}\n`, {
  ...
  async listEventsSince(since: Date, now: Date = new Date()): Promise<WideCollectorEvent[]> {
...
export class MinioRollupStore implements RollupStore {
  ...
  async appendRollups(records: WideRollupRecord[]): Promise<void> {
    await Promise.all(
      records.map((record) =>
        this.client.write(rollupKey(this.prefix, record), `${JSON.stringify(record)}\n`, {
          type: "application/x-ndjson",
        }),
      ),
    );
  ...
  listRollupKeys(prefixOverride?: string): Promise<string[]> {
    return listAllKeys(this.client, prefixOverride ?? this.prefix);
  }

  readRollupObject(key: string): Promise<string> {
