...
describe("shouldTriage", () => {
  ...
  test("rejects ok outcomes", () => {
    expect(
      shouldTriage(createRollup({ final_outcome: "ok", final_status_code: 200 }), {
        minStatusCode: 500,
      }),
    ).toBe(false);
  });
  ...
  test("rejects 4xx errors when threshold is 500", () => {
    expect(shouldTriage(createRollup({ final_status_code: 404 }), { minStatusCode: 500 })).toBe(
      false,
    );
  });

  test("rejects rollups with no status code", () => {
    expect(shouldTriage(createRollup({ final_status_code: null }), { minStatusCode: 500 })).toBe(
      false,
    );
  });
});
...
describe("DedupingTriageDispatcher", () => {
  ...
  test("filters out 2xx rollups", async () => {
    ...
    await dispatcher.dispatch(createRollup({ final_outcome: "ok", final_status_code: 200 }));
