...
describe("buildCanonicalRequest", () => {
  test("merges events from multiple services into one canonical request", () => {
    ...
    expect((canonical.request.request as Record<string, unknown>).path).toBe("/session/exchange");
    expect((canonical.request.response as Record<string, unknown>).content_type).toBe(
      "application/json",
    );
  });
