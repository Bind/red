...
export interface ActiveRequestAggregator {
  acceptBatch(batch: AcceptedCollectorBatch): Promise<WideRollupRecord[]> | WideRollupRecord[];
...
function validateEvent(input: unknown, index: number): EventValidationResult {
  ...
  const requestIdResult = requiredString(input.request_id, "request_id", eventId);
  ...
  const statusCode =
    typeof input.status_code === "number" && Number.isFinite(input.status_code)
      ...
      : undefined;
  const outcome = input.outcome === "ok" || input.outcome === "error" ? input.outcome : undefined;
...
export class InMemoryActiveRequestAggregator implements ActiveRequestAggregator {
...
export async function acceptCollectorBatch(
  ...
  if (batch.events.length > 0) {
    ...
    const requestIds = [...new Set(batch.events.map((event) => event.request_id))];
