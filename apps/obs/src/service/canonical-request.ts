...
function valuesEqual(left: ObsValue | undefined, right: ObsValue | undefined): boolean {
...
function appendConflict(target: ObsFields, key: string, left: ObsValue, right: ObsValue) {
...
function summarizeService(events: ObsEvent[]): ServiceSummary {
  ...
  const mergedData = events.reduce<ObsFields>((acc, event) => mergeFields(acc, event.data), {});
  const terminal = [...events]
    .reverse()
    .find((event) => event.status_code !== undefined || event.outcome !== undefined);
...
function collectRouteNames(events: ObsEvent[]): string[] {
  ...
  for (const event of events) {
    ...
    if (!isPlainObject(route) || typeof route.name !== "string" || route.name.length === 0) {
      continue;
    }
...
function hasTerminalEvent(event: ObsEvent): boolean {
  return Boolean(event.ended_at) || event.status_code !== undefined || event.outcome !== undefined;
}
...
function finalOutcome(events: ObsEvent[]): "ok" | "error" | "unknown" {
  ...
  return events.some((event) => event.outcome === "error" || (event.status_code ?? 0) >= 500)
    ? "error"
    : "ok";
}

function finalStatusCode(events: ObsEvent[]): number | null {
  const terminal = [...events].reverse().find((event) => event.status_code !== undefined);
...
export function buildCanonicalRequest(events: ObsEvent[]): CanonicalRequest {
  ...
  const request = ordered.reduce<ObsFields>((acc, event) => mergeFields(acc, event.data), {});
  ...
  return {
    ...
    first_ts: first.started_at,
    last_ts: last.ended_at ?? last.started_at,
    total_duration_ms: Date.parse(last.ended_at ?? last.started_at) - Date.parse(first.started_at),
