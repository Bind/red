...
fn handleRequest(
    ...
    var forwarded_for: ?[]const u8 = null;
    var origin: ?[]const u8 = null;
    var referer: ?[]const u8 = null;
    var content_type: ?[]const u8 = null;
    var request_id_header: ?[]const u8 = null;
    ...
    while (lines.next()) |line| {
        ...
        if (std.mem.startsWith(u8, line, "Host: ") or std.mem.startsWith(u8, line, "host: ")) {
            host_header = std.mem.trimStart(u8, line[6..], " \t");
        }
        if (std.mem.startsWith(u8, line, "User-Agent: ") or std.mem.startsWith(u8, line, "user-agent: ")) {
            user_agent = std.mem.trimStart(u8, line[12..], " \t");
        }
        if (std.mem.startsWith(u8, line, "X-Forwarded-For: ") or std.mem.startsWith(u8, line, "x-forwarded-for: ")) {
            forwarded_for = std.mem.trimStart(u8, line[17..], " \t");
        }
        if (std.mem.startsWith(u8, line, "Origin: ") or std.mem.startsWith(u8, line, "origin: ")) {
            origin = std.mem.trimStart(u8, line[8..], " \t");
        }
        if (std.mem.startsWith(u8, line, "Referer: ") or std.mem.startsWith(u8, line, "referer: ")) {
            referer = std.mem.trimStart(u8, line[9..], " \t");
        }
        if (std.mem.startsWith(u8, line, "Content-Type: ") or std.mem.startsWith(u8, line, "content-type: ")) {
            content_type = std.mem.trimStart(u8, line[14..], " \t");
        }
        if (std.mem.startsWith(u8, line, "X-Request-Id: ") or std.mem.startsWith(u8, line, "x-request-id: ")) {
            request_id_header = std.mem.trimStart(u8, line[14..], " \t");
        }
    }

    var obs = try obs_mod.RequestObs.init(
        alloc,
        io,
        method,
        raw_path,
        request_id_header,
        host_header,
        firstForwardedIp(forwarded_for),
        user_agent,
        origin,
        referer,
        content_type,
    ...
    if (parseControlPlaneRoute(path)) |route| {
        ...
        const repo_id = std.fmt.allocPrint(alloc, "{s}/{s}", .{ route.owner, route.repo }) catch {
            return sendPlainError(io, conn, &obs, "500 Internal Server Error", "internal control-plane error\n", keep_alive);
        };
        defer alloc.free(repo_id);
        obs.setRepo(repo_id, if (use_minio) "minio" else "disk");
        obs.setAuthRequired("read");

        const auth_error = authorizeRequest(alloc, auth_config, authorization, repo_id, .read, &obs) catch {
            return sendPlainError(io, conn, &obs, "500 Internal Server Error", "internal authentication error\n", keep_alive);
        };
        if (auth_error) |reason| {
            return sendUnauthorized(io, conn, &obs, reason, keep_alive);
        }
    ...
    if (std.mem.endsWith(u8, path, "/info/refs")) {
        obs.setRoute(.smart_http, "info_refs");
        const repo_id = maybe_repo_id orelse {
            return sendPlainError(io, conn, &obs, "404 Not Found", "Missing repo\n", keep_alive);
        };
        obs.setRepo(repo_id, if (use_minio) "minio" else "disk");
        const required_access = getRequiredAccess(method, path, raw_path);
        obs.setAuthRequired(accessName(required_access));
        const auth_error = authorizeRequest(alloc, auth_config, authorization, repo_id, required_access, &obs) catch {
            return sendPlainError(io, conn, &obs, "500 Internal Server Error", "internal authentication error\n", keep_alive);
        };
        if (auth_error) |reason| {
            return sendUnauthorized(io, conn, &obs, reason, keep_alive);
        ...
        const service: []const u8 = if (is_rp) "git-receive-pack" else if (is_up) "git-upload-pack" else {
            return sendPlainError(io, conn, &obs, "403 Forbidden", "Unsupported\n", keep_alive);
        };
        obs.setGitService(service);
        const resp = try protocol.advertiseRefs(alloc, repo_storage.adapter, service);
        defer alloc.free(resp);
        var ct: [80]u8 = undefined;
        const response_content_type = std.fmt.bufPrint(&ct, "application/x-{s}-advertisement", .{service}) catch "application/octet-stream";
        _ = sendResponse(io, conn, &obs, "200 OK", response_content_type, resp, keep_alive);
    } else if (std.mem.endsWith(u8, path, "/git-receive-pack") and std.mem.eql(u8, method, "POST")) {
        obs.setRoute(.smart_http, "receive_pack");
        const repo_id = maybe_repo_id orelse {
            return sendPlainError(io, conn, &obs, "404 Not Found", "Missing repo\n", keep_alive);
        };
        obs.setRepo(repo_id, if (use_minio) "minio" else "disk");
        obs.setGitService("git-receive-pack");
        obs.setAuthRequired("write");
        const auth_error = authorizeRequest(alloc, auth_config, authorization, repo_id, .write, &obs) catch {
            return sendPlainError(io, conn, &obs, "500 Internal Server Error", "internal authentication error\n", keep_alive);
        };
        if (auth_error) |reason| {
            return sendUnauthorized(io, conn, &obs, reason, keep_alive);
        ...
        defer alloc.free(resp);
        _ = sendResponse(io, conn, &obs, "200 OK", "application/x-git-receive-pack-result", resp, keep_alive);
    } else if (std.mem.endsWith(u8, path, "/git-upload-pack") and std.mem.eql(u8, method, "POST")) {
        obs.setRoute(.smart_http, "upload_pack");
        const repo_id = maybe_repo_id orelse {
            return sendPlainError(io, conn, &obs, "404 Not Found", "Missing repo\n", keep_alive);
        };
        obs.setRepo(repo_id, if (use_minio) "minio" else "disk");
        obs.setGitService("git-upload-pack");
        obs.setAuthRequired("read");
        const auth_error = authorizeRequest(alloc, auth_config, authorization, repo_id, .read, &obs) catch {
            return sendPlainError(io, conn, &obs, "500 Internal Server Error", "internal authentication error\n", keep_alive);
        };
        ...
        defer alloc.free(resp);
        _ = sendResponse(io, conn, &obs, "200 OK", "application/x-git-upload-pack-result", resp, keep_alive);
    } else {
        obs.setRoute(.root, null);
        ...
        _ = sendResponse(io, conn, &obs, "200 OK", "application/json", body_json, keep_alive);
    }
...
fn handleControlPlaneRequest(
    ...
    const result = switch (route.resource) {
        ...
        .file => blk: {
            ...
            const ref = queryParam(alloc, raw_path, "ref") catch null;
            defer if (ref) |value| alloc.free(value);
            const path_value = path orelse return sendJsonError(alloc, io, conn, obs, "Missing required query param: path", keep_alive);
            const ref_value = ref orelse "main";
        ...
        .compare => blk: {
            ...
            const base_value = base orelse return sendJsonError(alloc, io, conn, obs, "Missing required query params: base, head", keep_alive);
            const head_value = head orelse return sendJsonError(alloc, io, conn, obs, "Missing required query params: base, head", keep_alive);
        ...
        obs_mod.setError(obs, err);
        return sendJsonError(alloc, io, conn, obs, message, keep_alive);
    };

    defer alloc.free(result);
    p("[control-plane] response bytes={d}\n", .{result.len});
    _ = sendResponse(io, conn, obs, "200 OK", "application/json", result, keep_alive);
    return keep_alive;
}
...
fn authorizeRequest(
    ...
    return switch (decision) {
        .ok => blk: {
            obs.setAuthDecision("ok", null);
            break :blk null;
        },
        .deny => |reason| blk: {
            obs.setAuthDecision("deny", null);
            break :blk reason;
        },
    };
}
...
fn sendUnauthorized(io: Io, conn: *net.Stream, obs: *obs_mod.RequestObs, reason: []const u8, keep_alive: bool) bool {
...
fn sendPlainError(io: Io, conn: *net.Stream, obs: *obs_mod.RequestObs, status: []const u8, body: []const u8, keep_alive: bool) bool {
    return sendResponse(io, conn, obs, status, "text/plain", body, keep_alive);
}

fn sendJsonError(alloc: std.mem.Allocator, io: Io, conn: *net.Stream, obs: *obs_mod.RequestObs, message: []const u8, keep_alive: bool) bool {
    const json = buildErrorJson(alloc, message) catch return false;
    defer alloc.free(json);
    return sendResponse(io, conn, obs, "500 Internal Server Error", "application/json", json, keep_alive);
}
...
fn sendResponse(io: Io, conn: *net.Stream, obs: *obs_mod.RequestObs, status: []const u8, ct: []const u8, body: []const u8, keep_alive: bool) bool {
    return sendResponseWithHeaders(io, conn, obs, status, "", ct, body, keep_alive);
}
...
fn sendResponseWithHeaders(
    ...
    var h: [1024]u8 = undefined;
    const conn_hdr: []const u8 = if (keep_alive) "keep-alive" else "close";
    const status_code = parseStatusCode(status);
    obs_mod.setResponse(obs, status_code, ct);
    const hdr = std.fmt.bufPrint(
        &h,
        "HTTP/1.1 {s}\r\nx-request-id: {s}\r\n{s}Content-Type: {s}\r\nContent-Length: {d}\r\nCache-Control: no-cache\r\nConnection: {s}\r\n\r\n",
        .{ status, obs.requestId(), extra_headers, ct, body.len, conn_hdr },
    ) catch return false;
...
fn parseStatusCode(status: []const u8) u16 {
    const space = std.mem.indexOfScalar(u8, status, ' ') orelse return 500;
    return std.fmt.parseInt(u16, status[0..space], 10) catch 500;
}

fn accessName(access: auth_mod.RepoAccess) []const u8 {
...
fn controlPlaneResourceName(resource: ControlPlaneResource) []const u8 {
...
fn firstForwardedIp(value: ?[]const u8) ?[]const u8 {
    const raw = value orelse return null;
    const first = std.mem.indexOfScalar(u8, raw, ',') orelse raw.len;
    return std.mem.trim(u8, raw[0..first], " \t");
}
